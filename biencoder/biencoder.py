...
from biencoder.util import *
from biencoder.models import BiEncoderModel
...
def load_and_preprocess_glosses(data, tokenizer, max_len=-1):
	sense_glosses = {}
		...
		for word, label in sent:
			if label == -1:
				...
				if key not in sense_glosses:
					...
					gloss_arr = [s.definition() for s in syns]
					sensekey_arr = [s.lemmas()[0].key() for s in syns]
...
def preprocess_context(tokenizer, text_data, bsz=1, max_len=-1):
	...
	for sent in text_data:
		...
		for idx, (word, inst) in enumerate(sent):
	...
	#package data
	data = list(zip(context_ids, context_attn_masks, context_output_masks, words))
	return data
...
def do_eval(eval_data, model, gloss_dict, multigpu=False):
	model.eval()
	eval_preds = []
	for context_ids, context_attn_mask, context_output_mask, word in eval_data:
		word = word[0]
		with torch.no_grad(): 
			...
			eval_preds.append((word, pred_label))
